...
write.table(masked_cnv_breakpoints[masked_cnv_breakpoints$sample %in% LIHC_t10,c(2,3,4,5)], file="LIHC_t10_SVB",sep = "\t", append=FALSE,row.names = FALSE,col.names = FALSE, quote =FALSE)

# LUAD_t0
LUAD_t0 <- PRDM9.expression[PRDM9.expression$cancerType == "LUAD" & PRDM9.expression$PRDM9Expression > 0,]$sampleID
write.table(masked_cnv_breakpoints[masked_cnv_breakpoints$sample %in% LUAD_t0,c(2,3,4,5)], file="LUAD_t0_SVB",sep = "\t", append=FALSE,row.names = FALSE,col.names = FALSE, quote =FALSE)

# LUAD_t8
LUAD_t8 <- PRDM9.expression[PRDM9.expression$cancerType == "LUAD" & PRDM9.expression$PRDM9Expression > 8,]$sampleID
write.table(masked_cnv_breakpoints[masked_cnv_breakpoints$sample %in% LUAD_t8,c(2,3,4,5)], file="LUAD_t8_SVB",sep = "\t", append=FALSE,row.names = FALSE,col.names = FALSE, quote =FALSE)

# LUAD_t10
LUAD_t10 <- PRDM9.expression[PRDM9.expression$cancerType == "LUAD" & PRDM9.expression$PRDM9Expression > 10,]$sampleID
write.table(masked_cnv_breakpoints[masked_cnv_breakpoints$sample %in% LUAD_t10,c(2,3,4,5)], file="LUAD_t10_SVB",sep = "\t", append=FALSE,row.names = FALSE,col.names = FALSE, quote =FALSE)

# LUSC_t8
LUSC_t8 <- PRDM9.expression[PRDM9.expression$cancerType == "LUSC" & PRDM9.expression$PRDM9Expression > 8,]$sampleID
write.table(masked_cnv_breakpoints[masked_cnv_breakpoints$sample %in% LUSC_t8,c(2,3,4,5)], file="LUSC_t8_SVB",sep = "\t", append=FALSE,row.names = FALSE,col.names = FALSE, quote =FALSE)
...
write.table(masked_cnv_breakpoints[masked_cnv_breakpoints$sample %in% PRAD_t0,c(2,3,4,5)], file="PRAD_t0_SVB",sep = "\t", append=FALSE,row.names = FALSE,col.names = FALSE, quote =FALSE)

# STAD_t7
STAD_t7 <- PRDM9.expression[PRDM9.expression$cancerType == "STAD" & PRDM9.expression$PRDM9Expression > 7,]$sampleID
write.table(masked_cnv_breakpoints[masked_cnv_breakpoints$sample %in% STAD_t7,c(2,3,4,5)], file="STAD_t7_SVB",sep = "\t", append=FALSE,row.names = FALSE,col.names = FALSE, quote =FALSE)

# STAD_t9
STAD_t9 <- PRDM9.expression[PRDM9.expression$cancerType == "STAD" & PRDM9.expression$PRDM9Expression > 9,]$sampleID
write.table(masked_cnv_breakpoints[masked_cnv_breakpoints$sample %in% STAD_t9,c(2,3,4,5)], file="STAD_t9_SVB",sep = "\t", append=FALSE,row.names = FALSE,col.names = FALSE, quote =FALSE)

# STAD_t10
STAD_t10 <- PRDM9.expression[PRDM9.expression$cancerType == "STAD" & PRDM9.expression$PRDM9Expression > 10,]$sampleID
write.table(masked_cnv_breakpoints[masked_cnv_breakpoints$sample %in% STAD_t10,c(2,3,4,5)], file="STAD_t10_SVB",sep = "\t", append=FALSE,row.names = FALSE,col.names = FALSE, quote =FALSE)

# TGCT_t10
TGCT_t10 <- PRDM9.expression[PRDM9.expression$cancerType == "TGCT" & PRDM9.expression$PRDM9Expression > 10,]$sampleID
...
bedtools intersect \
-a /exports/eddie/scratch/s1949868/CopyNumber/SVB_cancerType_PRDM9Level/TGCT_t10_SVB \
-b /exports/eddie/scratch/s1949868/CompareCounts/sigIncre/TGCT*t10_sigIncre.txt \
-u | wc -l 
```
## Output
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTYwNTcxMDYyMSwtMTUwMDA1OTczMywtMj
kzNjY1ODE3LDEwNjY3MDIwMjMsLTEzMDMxODEwNTIsMTU0OTU2
NjQyMiwtMTg5MzY2ODMzNywtMTE2MjQ2NzY3MCwxNzYzODczOT
Y0LC01NzIyODIyODMsLTE5OTYzNzg2MTMsMjAwOTQwMzU4Nywt
OTk0MTI3OTE5LC0xNjczMTI1NjEwLC0zNDk0NDA4MDUsLTExMz
c1NDg3MjgsMjEyMTk0NDUwNiw5NDI0NzcxOTEsMTg3NTEyNTgw
NV19
-->